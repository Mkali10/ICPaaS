...
public sealed class ProvisioningWorker(PlatformStore store,ILogger<ProvisioningWorker> log):BackgroundService
{
    ...
    async Task<bool> Process(CancellationToken ct)
    {
        ...
        await using var r=await pick.ExecuteReaderAsync(ct);if(!await r.ReadAsync(ct)){await r.CloseAsync();await tx.RollbackAsync(ct);return false;}var id=r.GetGuid(0);var node=r.GetGuid(1);var trunk=r.GetGuid(2);var endpoint=r.GetString(3);var transport=r.GetString(4);await r.CloseAsync();
