...
public sealed record DidSettings(bool Inbound,bool OutboundCli);
public static class InfrastructureAdminEndpoints
{
 ...
 public static void MapInfrastructureAdmin(this WebApplication app)
 {
  ...
  api.MapPatch("/infrastructure/dids/{id:guid}",async(Guid id,DidSettings b,ClaimsPrincipal u,PlatformStore s,CancellationToken ct)=>{
   if(!Admin(u))return Results.Forbid();await using var c=await s.Open(ct);var sql="UPDATE dids SET use_for_inbound=$2,use_for_outbound_cli=$3,updated_at=now() WHERE id=$1"+(Platform(u)?"":" AND tenant_id=$4")+" RETURNING id,use_for_inbound,use_for_outbound_cli";await using var q=new NpgsqlCommand(sql,c);Add(q,id,b.Inbound,b.OutboundCli);if(!Platform(u))q.Parameters.AddWithValue(Tenant(u));var row=await One(q,ct);return row is null?Results.NotFound():Results.Ok(row);
  });
 }
